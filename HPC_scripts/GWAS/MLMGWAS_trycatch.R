...
for(i in 2:ncol(pheno)) {
  tryCatch({
    cat("Processing phenotype column:", colnames(pheno)[i], "\n")
    ...
    imMVP <- MVP(
      ...
      method = "MLM",
      method.bin = "static",
      p.threshold = (0.05 / EFFECTIVE_MARKERS), 
      threshold = (0.05 / effective_ratio),
      file.output = 'pmap.signal', #Exporting significant only SNP output file
      vc.method = "BRENT",
      ncpus = 24
    )
    
  }, error = function(e) {
    cat("Error processing phenotype:", colnames(pheno)[i], "\n")
    cat("Error message:", conditionMessage(e), "\n")
    failed_phenotypes <<- c(failed_phenotypes, colnames(pheno)[i])
  })
  ...
  tryCatch({
    ...
    imMVP <- MVP(
      ...
      p.threshold = (0.05 / EFFECTIVE_MARKERS), 
      threshold = (0.05 / effective_ratio),
      file.output = 'pmap', #Exporting whole SNP output file
      vc.method = "BRENT",
      ncpus = 24
    )
...
# Optionally write the failed phenotypes to a file
if(length(failed_phenotypes) > 0) {
  writeLines(failed_phenotypes, "failed_phenotypes.txt")
  cat("Skipped phenotypes were saved to failed_phenotypes.txt\n")
}
